var dialog = (function Dialog() {
    ...
    //title--标题
    //content--对象{type[1 for input, 0 for text], text[display in content]}
    //btnFlag--1 for 确定+取消, 0 for 确定
    function initHtml(title, content, btnFlag, ok, cancel) {
        var div = "";
        div += "<div id='dlg-box' class='dot'>";
        div += "<h1 class='dot'>" + title + "</h1>"
        if (content.type == 0) {
        ...
        if (btnFlag == 0) {
            div += "<div><span class='ok'>确定</span></div>";
        } else if (btnFlag == 1) {
            div += "<div><span class='ok'>确定</span><span class='cancel'>取消</span></div>";
        }
        div += "</div>";
        $mask.html(div);
